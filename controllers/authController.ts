...
const setRefreshTokenCookie = (res: Response, token: string) => {
  res.cookie('refreshToken', token, {
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production', // solo seguro en prod
    sameSite: 'none', // obligatorio para cross-origin cookies
...
export const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existingUser) return res.status(409).json({ message: 'El correo electrónico ya está registrado.' });
